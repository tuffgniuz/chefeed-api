...
async def create_review(review: Review, recipe_id: PydanticObjectId, current_user=Depends(current_active_user)) -> dict:
    ...
    if word_count > 150:
        raise HTTPException(
            status_code=400, detail='Review must be shorter than 150 words!')

    text_processed = text_preprocessing(text)
    model = load_model(
        '/src/app/machine_learning/lstm_embed_tuned_valloss_ext2.h5')
    ...
    if review is None:
        raise HTTPException(
            status_code=404, detail=f"Recipe does not exist with id {recipe_id}")
